...
async function refreshForecastFromSavedLocation(entries){
  if(!forecast5DayEl) return;
  forecast5DayEl.innerHTML = '<p class="empty">Loading forecast…</p>';
  try {
    const saved = loadSavedLocation();
    const loc = saved?.matchedLocation || matchedLocation || entries?.at(-1)?.location;
    if(!loc || !loc.latitude || !loc.longitude){
      forecast5DayEl.innerHTML = '<p class="empty">Set your location to see a 5-day migraine risk forecast.</p>';
      return;
    }
    matchedLocation = loc;
    ...
    try { renderDBForecastStrip(forecastCache); } catch(e){ console.error('DB forecast strip failed', e); }
  } catch (err) {
...
async function renderFiveDayForecast(entries){
  return refreshForecastFromSavedLocation(entries);
  try {
...
/* ── Dashboard helpers ── */
const dbAvg = arr => { const v=(arr||[]).filter(x=>x!=null&&!isNaN(x)); return v.length?v.reduce((a,b)=>a+b,0)/v.length:null; };
const dbRiskClass = s => s>=7.5?'very-high':s>=5.5?'high':s>=3.2?'moderate':'low';
const dbRiskLabel = s => s>=7.5?'Very High':s>=5.5?'High':s>=3.2?'Moderate':'Low';
let _dbChartPressure=null, _dbChartSleep=null;
...
function renderDashboard(entries){
  ...
  _renderDBHero(entries);
}

function _renderDBStats(entries){
  const now=new Date();
  const mk=`${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;
  const tm=entries.filter(e=>e.date&&e.date.startsWith(mk));
  const dim=new Date(now.getFullYear(),now.getMonth()+1,0).getDate();
  const md=tm.filter(e=>Number(e.severity)>=1).length;
  const as=tm.length?dbAvg(tm.map(e=>Number(e.severity))):null;
  const hs=entries.filter(e=>Number(e.severity)>=6);
  const pd=hs.filter(e=>(e.weather?.pressureChange??0)<=-3).length;
  const pc=hs.length?Math.round((pd/hs.length)*100):null;
  const g=id=>{const el=document.getElementById(id);return el;};
  if(g('ds-migraine-days')) g('ds-migraine-days').textContent=md;
  if(g('ds-avg-sev'))       g('ds-avg-sev').textContent=as!=null?as.toFixed(1):'—';
  if(g('ds-free-days'))     g('ds-free-days').textContent=dim-md;
  if(g('ds-pressure-trigger')) g('ds-pressure-trigger').textContent=pc!=null?`${pc}%`:'—';
}
...
function _renderDBTriggers(entries){
  ...
  const c={};
  entries.forEach(e=>(e.triggers||[]).forEach(t=>{c[t]=(c[t]||0)+1;}));
  const s=Object.entries(c).sort((a,b)=>b[1]-a[1]).slice(0,6);
  if(!s.length){el.innerHTML='<div class="empty">No triggers logged yet.</div>';return;}
  const max=s[0][1];
  el.innerHTML=s.map(([n,cnt])=>`<div class="db-trigger-row"><span class="db-trigger-name">${n}</span><div class="db-trigger-track"><div class="db-trigger-fill" style="width:${Math.round((cnt/max)*100)}%"></div></div><span class="db-trigger-count">${cnt}</span></div>`).join('');
}
...
function _renderDBInsights(entries){
  ...
  if(!el) return;
  const ins=[];
  const r=entries.slice(-30);
  const hs=r.filter(e=>Number(e.severity)>=6);
  const dm=hs.filter(e=>(e.weather?.pressureChange??0)<=-3);
  if(hs.length>=3){ const p=Math.round((dm.length/hs.length)*100); ins.push({icon:'📉',strong:`${p}% pressure correlation.`,text:`${dm.length} of your last ${hs.length} high-severity migraines occurred on days with a pressure drop ≥ 3 hPa.`,sub:'Based on your last 30 entries'}); }
  const rd=hs.filter(e=>e.weather?.rain).length;
  if(hs.length>=2&&rd>0) ins.push({icon:'🌧',strong:'Rain as a trigger.',text:`Rain was present on ${rd} of your high-severity days. Monitor rain forecasts as an early warning.`,sub:null});
  const se=entries.filter(e=>e.sleep&&e.severity);
  if(se.length>=5){ const ls=se.filter(e=>Number(e.sleep)<6&&Number(e.severity)>=5); if(ls.length>=2) ins.push({icon:'🌙',strong:'Sleep under 6 hours linked to higher severity.',text:`${ls.length} entries show severity ≥ 5 after fewer than 6 hours of sleep.`,sub:null}); }
  const hd=hs.filter(e=>(e.weather?.humidity??0)>75);
  if(hd.length>=2) ins.push({icon:'💧',strong:'High humidity pattern.',text:`${hd.length} high-severity entries had humidity above 75%. Stay hydrated on humid days.`,sub:null});
  if(!ins.length){el.innerHTML='<div class="empty">Log more entries to unlock pattern insights.</div>';return;}
  el.innerHTML=ins.map(i=>`<div class="db-insight"><span class="db-insight-icon">${i.icon}</span><div class="db-insight-text"><strong>${i.strong}</strong> ${i.text}${i.sub?`<span class="db-sub">${i.sub}</span>`:''}</div></div>`).join('');
}

function _renderDBCharts(entries){
  const l14=entries.slice(-14);
  const blue='#4a7ab5', coral='#c97060', muted='rgba(100,80,60,0.35)', grid='rgba(100,80,60,0.1)', tick='#9c8878';
  const sc={ticks:{color:tick,font:{size:10}},grid:{color:grid}};
  const tip={backgroundColor:'rgba(255,252,248,0.95)',titleColor:'#3d342d',bodyColor:'#6c5f51',borderColor:'rgba(201,176,146,0.5)',borderWidth:1};
  const pc=document.getElementById('db-chart-pressure');
  if(pc){
    if(_dbChartPressure){_dbChartPressure.destroy();_dbChartPressure=null;}
    _dbChartPressure=new Chart(pc,{type:'line',data:{labels:l14.map(e=>e.date?e.date.slice(5):''),datasets:[{label:'Pressure hPa',data:l14.map(e=>e.weather?.pressure??null),yAxisID:'yP',borderColor:blue,backgroundColor:'rgba(74,122,181,0.07)',tension:0.35,pointRadius:2,borderDash:[4,3],spanGaps:true},{label:'Severity',data:l14.map(e=>Number(e.severity)||0),yAxisID:'yS',borderColor:coral,backgroundColor:'rgba(201,112,96,0.07)',tension:0.35,pointRadius:2,spanGaps:true}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:tip},scales:{x:{...sc,ticks:{...sc.ticks,maxTicksLimit:7,autoSkip:true}},yP:{type:'linear',position:'left',ticks:{color:blue,font:{size:10}},grid:{color:grid}},yS:{type:'linear',position:'right',min:0,max:10,ticks:{color:coral,font:{size:10}},grid:{display:false}}}}});
  }
  const slc=document.getElementById('db-chart-sleep');
  if(slc){
    if(_dbChartSleep){_dbChartSleep.destroy();_dbChartSleep=null;}
    const sd=entries.filter(e=>e.sleep&&e.severity).map(e=>({x:Number(e.sleep),y:Number(e.severity)}));
    _dbChartSleep=new Chart(slc,{type:'scatter',data:{datasets:[{label:'Sleep vs severity',data:sd,backgroundColor:'rgba(74,122,181,0.5)',borderColor:blue,pointRadius:5}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{...tip,callbacks:{label:ctx=>`${ctx.parsed.x}h sleep · severity ${ctx.parsed.y}`}}},scales:{x:{...sc,min:2,max:12,title:{display:true,text:'Sleep (hrs)',color:tick,font:{size:10}}},y:{...sc,min:0,max:10,title:{display:true,text:'Severity',color:tick,font:{size:10}}}}}});
  }
}

function _renderDBHero(entries){
  const loc=document.getElementById('db-location');
  const num=document.getElementById('db-score-num');
  const pill=document.getElementById('db-risk-pill');
  const saved=(()=>{try{return JSON.parse(localStorage.getItem('mwt_saved_location_v1'));}catch{return null;}})();
  if(loc) loc.textContent=saved?.matchedLocation?`${saved.matchedLocation.name}, ${saved.matchedLocation.country}`:'Set location in Log Entry tab';
  ...
  const w=te?.weather;
  const setW=(id,val,unit='')=>{const el=document.getElementById(id);if(el)el.textContent=val!=null?`${Number(val).toFixed(0)}${unit}`:'—';};
  setW('dw-pressure',w?.pressure,' hPa');
  setW('dw-humidity',w?.humidity,'%');
  setW('dw-temp',w?.temperature,'°C');
  const cl=document.getElementById('dw-cloud');
  if(cl) cl.textContent=w?.rain?'Rain':'—';
}

function renderDBForecastStrip(fc){
  const el=document.getElementById('db-forecast-strip');
  if(!el||!fc||!Object.keys(fc).length) return;
  const icon=l=>{const x=l.toLowerCase();return x.includes('very')||x.includes('high')?'⛈':x.includes('moderate')?'🌥':'🌤';};
  el.innerHTML=Object.entries(fc).slice(0,5).map(([date,a])=>{
    const dn=new Date(date).toLocaleDateString(undefined,{weekday:'short'});
    const rc=a.level.toLowerCase().replace(' ','-');
    const ch=a.total24hDrop>=0?`+${a.total24hDrop.toFixed(1)}`:`${a.total24hDrop.toFixed(1)}`;
    return `<div class="db-fday"><div class="db-fday-name">${dn}</div><div class="db-fday-icon">${icon(a.level)}</div><div class="db-fday-pressure">${(a.total24hDrop+1013).toFixed(0)} hPa</div><div class="db-fday-change">${ch} hPa</div><span class="db-fday-risk ${rc}">${a.level}</span></div>`;
  }).join('');
}
function refresh(){ const entries=loadEntries(); renderEntries(entries); renderDashboard(entries); renderTrendCharts(entries); try { renderCalendar(entries); } catch (err) { console.error('Calendar module failed during refresh (non-blocking)', err); } try { refreshForecastFromSavedLocation(entries); } catch(err){ console.error('Forecast render failed (non-blocking)', err); } applyMoodState(entries); }
...
bindTabs();
bindChartModal();
if(refreshForecastBtn) refreshForecastBtn.addEventListener('click', ()=>{ refreshForecastBtn.disabled=true; refreshForecastBtn.textContent='Refreshing…'; refreshForecastFromSavedLocation(loadEntries()).finally(()=>{ refreshForecastBtn.disabled=false; refreshForecastBtn.textContent='Refresh forecast'; refresh(); }); });
refresh();
...
function bindTabs(){
  ...
  tabs.forEach((tab)=>tab.addEventListener('click', ()=>{
    ...
    if(tab.dataset.tab==='calendar' || tab.dataset.tab==='dashboard'){ try { refreshForecastFromSavedLocation(loadEntries()); } catch(e){ console.error('Tab-triggered forecast refresh failed', e); } }
  }));
}
